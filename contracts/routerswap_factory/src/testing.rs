...
use routerswap::mock_querier::{mock_dependencies, WasmMockQuerier};
...
use routerswap::asset::{AssetInfo, PairInfo};
use routerswap::factory::{
    ConfigResponse, ExecuteMsg, InstantiateMsg, NativeTokenDecimalsResponse, QueryMsg,
};
use routerswap::pair::{InstantiateMsg as PairInstantiateMsg, MigrateMsg as PairMigrateMsg};
...
fn create_pair() {
    ...
    deps = init(deps);
    deps.querier
        .with_routerswap_factory(&[], &[("uusd".to_string(), 6u8)]);
    let asset_infos = [
    ...
    let msg = ExecuteMsg::CreatePair {
        ...
        token_name: "Testing LP Token".into(),
        token_symbol: "uTLP".into(),
        token_decimal: 18,
    };
...
fn reply_test() {
    ...
    // register routerswap pair querier
    deps.querier.with_routerswap_factory(
        &[(
            &"0000".to_string(),
            &PairInfo {
