...
use routerswap::asset::{Asset, AssetInfo, PairInfo};
use routerswap::pair::SimulationResponse;
use routerswap::querier::{query_pair_info, reverse_simulate, simulate};
use routerswap::router::{
...
const CONTRACT_NAME: &str = "crates.io:routerswap-router";
...
pub fn instantiate(
    ...
    CONFIG.save(
        deps.storage,
        &Config {
            routerswap_factory: deps.api.addr_canonicalize(&msg.routerswap_factory)?,
        },
    )?;
...
pub fn query_config(deps: Deps) -> StdResult<ConfigResponse> {
    ...
    let resp = ConfigResponse {
        routerswap_factory: deps
            .api
            .addr_humanize(&state.routerswap_factory)?
            .to_string(),
    };
...
fn simulate_swap_operations(
    ...
    let routerswap_factory = deps.api.addr_humanize(&config.routerswap_factory)?;
    ...
    for operation in operations.into_iter() {
        match operation {
            SwapOperation::RouterSwap {
                offer_asset_info,
                ask_asset_info,
            } => {
                let pair_info: PairInfo = query_pair_info(
                    &deps.querier,
                    routerswap_factory.clone(),
